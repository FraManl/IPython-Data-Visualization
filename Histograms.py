...
data = np.genfromtxt ('D:/work/dyna_tempo.txt', delimiter="\t", skip_header=1)
patents = np.array(data[:,1], int)
years = np.array(data[:,0], int)
...
ax.set_xlabel("x label", size = 13)
ax.set_ylabel('y label', size = 13)
ax.set_xticklabels(years) # tickles for X axis
ax.grid(False) #no grid
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
...
def autolabel(rects, ax):
    ...
    for rect in rects:
        ...
        ax.text(rect.get_x() + rect.get_width()/2., label_position,
                '%d' % int(height), ha='center', va='bottom', rotation=-10, fontsize=8)
